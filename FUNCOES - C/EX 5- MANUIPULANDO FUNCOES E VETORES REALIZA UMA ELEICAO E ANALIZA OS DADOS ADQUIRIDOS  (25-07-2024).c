...
/*
Na sua turma será realizada a eleição para representante de sala, as turmas tem no máximo 50 alunos, os candida
tos foram: Marcos da Silva e a Ana Claudia de Alcantara. Para controlar a eleição deve ser implementado um pro
grama em linguagem C capaz de representar uma urna eletrônica, onde todos os alunos da turma poderão votar,
sendo que as opções são M – Marcos da Silva, A - Ana Claudia de Alcantara, B - Branco, X – para finalizar e qualquer
outro valor é um voto nulo. Após a finalização da eleição deve-se apresentar um resumo contendo:
...
f)
Percentual de votos válidos, ou seja, sem contar brancos e nulos;
g) Percentual de votos de cada um dos candidatos, considerando apenas os votos válidos;
h) Caso a quantidade de votos válidos não alcance 50% deve-se apresentar a mensagem “A eleição não foi válida”
i)
Se a eleição for válida apresentar o nome do novo representante de sala ou se houve empate.
*/
