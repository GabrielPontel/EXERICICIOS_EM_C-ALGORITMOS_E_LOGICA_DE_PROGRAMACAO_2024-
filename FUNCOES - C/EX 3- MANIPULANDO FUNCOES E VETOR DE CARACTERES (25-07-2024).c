#include <stdio.h>
/*
Fazer um programa em linguagem C utilizando funções, que trabalhe com vetores de caracteres de até 50 posições
e seja capaz de:
a) Excluir um caracter de uma determinada posição;
b) Incluir um caracter em uma determinada posição;
c) Copiar uma parte dos caracteres para dentro de outro vetor, para isso deve-se conhecer a posição inicial e o fi
nal a ser copiada;
d) Copiar a última palavra para outro vetor;
e) Copiar a primeira palavra para outro vetor;
f)
Retirar os espaços em branco repetidos;
g) Contar a quantidade de palavras;
h) Contar a quantidade de vogais.
*/
